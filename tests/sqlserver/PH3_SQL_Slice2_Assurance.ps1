...
traceability:
  ...
  approvals:
    - "Test Authority approval for exact Slice 2 commit and isolated localhost SQL Express Retry01 assurance: https://github.com/onkarpathre/lgr-transformation-migration/pull/8#pullrequestreview-5221828015"

Tester-owned, manual SQL Server runtime assurance harness for PH3-SQL-001 Slice 2.
...
This execution is pinned to the exact approved Retry01 database and uses an ordinal
case-sensitive equality gate. The script first proves non-existence through a
parameterised DB_ID check and never drops, deletes or reuses any database. A later
RetryNN requires separate authority and a Tester-owned source update.
#>
...
$ExpectedDatabase = 'LgrTransformationMigration_Ph3Sql_Slice2_Assurance_20260916_Retry01'
$ExpectedBranch = 'feature/ph3-sql-remaining-implementation'
$ExpectedHead = '7cd65a94163c2183c2ecb19d6e07d3a1d6c26920'
$ExpectedSqlVersion = '16.0.1000.6'
$PreviousMigrationBoundary = '20260910082037_AddInternalPrincipalAuditType'
$ExpectedMigrations = @(
...
$ResultsDirectory = Join-Path $RepositoryRoot (Join-Path 'TestResults' ("PH3_SQL_Slice2_Assurance_{0}" -f $RunId))
...
$Result = [ordered]@{
    schemaVersion = '1.0'
    workItem = 'PH3-SQL-001-slice-2-sql-server-runtime-assurance'
...
function New-ConnectionString {
    ...
    $builder['Application Name'] = 'PH3-SQL-001 Slice2 Assurance'
...
function Assert-EfNoPendingMigrations {
    ...
    Write-Evidence -Status PASS -Message 'EF reports all six migrations applied and no pending migration.'
}
...
function Assert-Slice2Schema {
    param([Parameter(Mandatory = $true)][string] $Phase)

    $tables = @(Invoke-SqlQuery -Sql @'
SELECT name AS TableName
FROM sys.tables
WHERE name IN (N'SqlInstanceDiscoverySnapshots', N'SqlDatabaseDiscoverySnapshots')
ORDER BY name;
'@)
    Assert-ExactSet -Name 'Slice 2 snapshot tables' `
        -Expected @('SqlInstanceDiscoverySnapshots', 'SqlDatabaseDiscoverySnapshots') `
        -Actual @($tables | ForEach-Object { [string] $_.TableName })

    $batchColumns = @(Get-TableColumns -QualifiedTable 'dbo.ImportBatches' | Where-Object {
        $_.ColumnName -in @('CommitIdempotencyKeyHash', 'CommitResultJson', 'RowVersion')
    })
    Assert-ExactSet -Name 'Slice 2 ImportBatches columns' `
        -Expected @('CommitIdempotencyKeyHash', 'CommitResultJson', 'RowVersion') `
        -Actual @($batchColumns | ForEach-Object { [string] $_.ColumnName })
    $batchRowVersion = $batchColumns | Where-Object { $_.ColumnName -ceq 'RowVersion' }
    Assert-Condition -Condition ($batchRowVersion.DataType -in @('timestamp', 'rowversion')) -Message 'ImportBatches.RowVersion is not SQL Server rowversion/timestamp.'
    Assert-Condition -Condition (-not [bool] $batchRowVersion.IsNullable) -Message 'ImportBatches.RowVersion is unexpectedly nullable.'

    $rowColumns = @(Get-TableColumns -QualifiedTable 'dbo.DiscoveryImportRows' | Where-Object {
        $_.ColumnName -in @(
            'MatchedSqlDatabaseId', 'MatchedSqlInstanceId', 'NormalizedDatabaseName',
            'NormalizedInstanceName', 'ProposedAction', 'ReconciliationFingerprint')
    })
    Assert-ExactSet -Name 'Slice 2 staging columns' `
        -Expected @('MatchedSqlDatabaseId', 'MatchedSqlInstanceId', 'NormalizedDatabaseName', 'NormalizedInstanceName', 'ProposedAction', 'ReconciliationFingerprint') `
        -Actual @($rowColumns | ForEach-Object { [string] $_.ColumnName })

    $requiredForeignKeys = @(
        'FK_DiscoveryImportRows_ImportBatches_CustomerId_ProjectId_ImportBatchId',
        'FK_DiscoveryImportRows_SqlDatabases_CustomerId_ProjectId_MatchedSqlDatabaseId',
        'FK_DiscoveryImportRows_SqlInstances_CustomerId_ProjectId_MatchedSqlInstanceId',
        'FK_SqlDatabaseDiscoverySnapshots_ImportBatches_CustomerId_ProjectId_ImportBatchId',
        'FK_SqlDatabaseDiscoverySnapshots_SqlDatabases_CustomerId_ProjectId_SqlDatabaseId',
        'FK_SqlInstanceDiscoverySnapshots_ImportBatches_CustomerId_ProjectId_ImportBatchId',
        'FK_SqlInstanceDiscoverySnapshots_SqlInstances_CustomerId_ProjectId_SqlInstanceId'
    )
    $foreignKeys = @(Invoke-SqlQuery -Sql @'
SELECT name AS ForeignKeyName
FROM sys.foreign_keys
WHERE name IN
(
    N'FK_DiscoveryImportRows_ImportBatches_CustomerId_ProjectId_ImportBatchId',
    N'FK_DiscoveryImportRows_SqlDatabases_CustomerId_ProjectId_MatchedSqlDatabaseId',
    N'FK_DiscoveryImportRows_SqlInstances_CustomerId_ProjectId_MatchedSqlInstanceId',
    N'FK_SqlDatabaseDiscoverySnapshots_ImportBatches_CustomerId_ProjectId_ImportBatchId',
    N'FK_SqlDatabaseDiscoverySnapshots_SqlDatabases_CustomerId_ProjectId_SqlDatabaseId',
    N'FK_SqlInstanceDiscoverySnapshots_ImportBatches_CustomerId_ProjectId_ImportBatchId',
    N'FK_SqlInstanceDiscoverySnapshots_SqlInstances_CustomerId_ProjectId_SqlInstanceId'
);
'@)
    Assert-ExactSet -Name 'Slice 2 composite foreign keys' -Expected $requiredForeignKeys -Actual @($foreignKeys | ForEach-Object { [string] $_.ForeignKeyName })

    $stagingBatchForeignKey = @(Get-ForeignKeyRows -QualifiedTable 'dbo.DiscoveryImportRows' | Where-Object {
        $_.ConstraintName -ceq 'FK_DiscoveryImportRows_ImportBatches_CustomerId_ProjectId_ImportBatchId'
    } | Sort-Object ColumnOrder)
    Assert-Condition -Condition ((@($stagingBatchForeignKey | ForEach-Object { [string] $_.ParentColumn }) -join ',') -ceq 'CustomerId,ProjectId,ImportBatchId') -Message 'The staging-to-batch foreign key is not tenant/project leading.'
    Assert-Condition -Condition ((@($stagingBatchForeignKey | ForEach-Object { [string] $_.ReferencedColumn }) -join ',') -ceq 'CustomerId,ProjectId,Id') -Message 'The staging-to-batch foreign key does not target the tenant/project-leading batch key.'
    Assert-Condition -Condition (("{0}.{1}" -f $stagingBatchForeignKey[0].ReferencedSchema, $stagingBatchForeignKey[0].ReferencedTable) -ceq 'dbo.ImportBatches') -Message 'The staging-to-batch foreign key targets the wrong table.'
    Assert-Condition -Condition ([string] $stagingBatchForeignKey[0].DeleteAction -ceq 'CASCADE') -Message 'The approved batch-owned staging cascade is absent.'
    Assert-Condition -Condition (-not [bool] $stagingBatchForeignKey[0].IsDisabled -and -not [bool] $stagingBatchForeignKey[0].IsNotTrusted) -Message 'The staging-to-batch foreign key is disabled or untrusted.'

    $requiredChecks = @(
        'CK_SqlDatabaseDiscoverySnapshots_CompatibilityLevel',
        'CK_SqlDatabaseDiscoverySnapshots_SizeMb',
        'CK_SqlInstanceDiscoverySnapshots_Port'
    )
    $checks = @(Invoke-SqlQuery -Sql @'
SELECT name AS CheckName
FROM sys.check_constraints
WHERE name IN
(
    N'CK_SqlDatabaseDiscoverySnapshots_CompatibilityLevel',
    N'CK_SqlDatabaseDiscoverySnapshots_SizeMb',
    N'CK_SqlInstanceDiscoverySnapshots_Port'
);
'@)
    Assert-ExactSet -Name 'Slice 2 snapshot checks' -Expected $requiredChecks -Actual @($checks | ForEach-Object { [string] $_.CheckName })

    $requiredIndexes = @(
        'IX_DiscoveryImportRows_CustomerId_ProjectId_ImportBatchId_RowNumber',
        'IX_DiscoveryImportRows_CustomerId_ProjectId_MatchedSqlDatabaseId',
        'IX_DiscoveryImportRows_Owner_DatabaseMatch',
        'IX_DiscoveryImportRows_Owner_InstanceMatch',
        'IX_ImportBatches_Owner_CommitIdempotencyKeyHash',
        'IX_SqlDatabaseDiscoverySnapshots_CustomerId_ProjectId_ImportBatchId',
        'IX_SqlDatabaseDiscoverySnapshots_Owner_History',
        'IX_SqlInstanceDiscoverySnapshots_CustomerId_ProjectId_ImportBatchId',
        'IX_SqlInstanceDiscoverySnapshots_Owner_History'
    )
    $indexes = @(Invoke-SqlQuery -Sql @'
SELECT name AS IndexName
FROM sys.indexes
WHERE name IN
(
    N'IX_DiscoveryImportRows_CustomerId_ProjectId_ImportBatchId_RowNumber',
    N'IX_DiscoveryImportRows_CustomerId_ProjectId_MatchedSqlDatabaseId',
    N'IX_DiscoveryImportRows_Owner_DatabaseMatch',
    N'IX_DiscoveryImportRows_Owner_InstanceMatch',
    N'IX_ImportBatches_Owner_CommitIdempotencyKeyHash',
    N'IX_SqlDatabaseDiscoverySnapshots_CustomerId_ProjectId_ImportBatchId',
    N'IX_SqlDatabaseDiscoverySnapshots_Owner_History',
    N'IX_SqlInstanceDiscoverySnapshots_CustomerId_ProjectId_ImportBatchId',
    N'IX_SqlInstanceDiscoverySnapshots_Owner_History'
);
'@)
    Assert-ExactSet -Name 'Slice 2 indexes' -Expected $requiredIndexes -Actual @($indexes | ForEach-Object { [string] $_.IndexName })

    $stagingRowIndex = @(Get-IndexRows -QualifiedTable 'dbo.DiscoveryImportRows' | Where-Object {
        $_.IndexName -ceq 'IX_DiscoveryImportRows_CustomerId_ProjectId_ImportBatchId_RowNumber'
    } | Sort-Object ColumnOrder)
    Assert-Condition -Condition ((@($stagingRowIndex | ForEach-Object { [string] $_.ColumnName }) -join ',') -ceq 'CustomerId,ProjectId,ImportBatchId,RowNumber') -Message 'The staging row uniqueness index is not owner leading.'
    Assert-Condition -Condition ([bool] $stagingRowIndex[0].IsUnique -and -not [bool] $stagingRowIndex[0].HasFilter) -Message 'The owner-leading staging row index is not unfiltered and unique.'

    $descendingCount = [int] (Invoke-SqlScalar -Sql @'
SELECT COUNT_BIG(*)
FROM sys.indexes AS i
INNER JOIN sys.index_columns AS ic ON ic.object_id = i.object_id AND ic.index_id = i.index_id
INNER JOIN sys.columns AS c ON c.object_id = ic.object_id AND c.column_id = ic.column_id
WHERE i.name IN (N'IX_SqlDatabaseDiscoverySnapshots_Owner_History', N'IX_SqlInstanceDiscoverySnapshots_Owner_History')
  AND c.name = N'ImportedAt'
  AND ic.is_descending_key = 1;
'@)
    Assert-Condition -Condition ($descendingCount -eq 2) -Message 'Both Slice 2 history indexes must order ImportedAt descending.'

    $inspection = [ordered]@{
        phase = $Phase
        snapshotTables = @($tables | ForEach-Object { [string] $_.TableName })
        batchColumns = @($batchColumns | ForEach-Object { [string] $_.ColumnName })
        stagingColumns = @($rowColumns | ForEach-Object { [string] $_.ColumnName })
        foreignKeys = @($foreignKeys | ForEach-Object { [string] $_.ForeignKeyName })
        checkConstraints = @($checks | ForEach-Object { [string] $_.CheckName })
        indexes = @($indexes | ForEach-Object { [string] $_.IndexName })
        descendingHistoryIndexes = $descendingCount
    }
    $null = $SchemaInspections.Add($inspection)
    Write-Evidence -Status PASS -Message ("{0}: Slice 2 rowversion, staging, snapshot, composite-FK, check and index schema is present." -f $Phase)
    return $inspection
}

function Invoke-Slice2RuntimeTests {
    param(
        [Parameter(Mandatory = $true)][hashtable] $Ids
    )

    $batchA = [guid] '94000000-0000-0000-0000-000000000001'
    $batchB = [guid] '94000000-0000-0000-0000-000000000002'
    $instanceSnapshot = [guid] '94000000-0000-0000-0000-000000000011'
    $databaseSnapshot = [guid] '94000000-0000-0000-0000-000000000012'
    $stagingRow = [guid] '94000000-0000-0000-0000-000000000021'
    $idempotencyHash = ('a' * 64)
    $batchSql = @'
INSERT INTO [dbo].[ImportBatches]
([Id], [CustomerId], [ProjectId], [SourceType], [OriginalFileName], [StoredFileName], [FileHash], [FileSizeBytes], [Status], [UploadedBy], [UploadedAt], [PreviewedAt], [CommittedAt], [TotalRows], [ValidRows], [CreateCount], [UpdateCount], [UnchangedCount], [WarningCount], [RejectCount], [Notes], [CommitIdempotencyKeyHash], [CommitResultJson])
VALUES
(@Id, @CustomerId, @ProjectId, N'SqlInstanceCsv/v1', N'synthetic.csv', NULL, @FileHash, 128, N'PreviewReady', N'local-test:tester', SYSDATETIMEOFFSET(), SYSDATETIMEOFFSET(), NULL, 1, 1, 1, 0, 0, 0, 0, NULL, @IdempotencyHash, N'{"safe":true}');
'@
    foreach ($batch in @(
        [ordered]@{ Id = $batchA; CustomerId = $Ids.CustomerA; ProjectId = $Ids.ProjectA1; Hash = ('1' * 64) },
        [ordered]@{ Id = $batchB; CustomerId = $Ids.CustomerB; ProjectId = $Ids.ProjectB1; Hash = ('2' * 64) }
    )) {
        $affected = Invoke-SqlNonQuery -Sql $batchSql -Parameters @(
            (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $batch.Id),
            (New-SqlParameterSpec -Name '@CustomerId' -Type UniqueIdentifier -Value $batch.CustomerId),
            (New-SqlParameterSpec -Name '@ProjectId' -Type UniqueIdentifier -Value $batch.ProjectId),
            (New-SqlParameterSpec -Name '@FileHash' -Type NChar -Value $batch.Hash -Size 64),
            (New-SqlParameterSpec -Name '@IdempotencyHash' -Type NChar -Value $idempotencyHash -Size 64)
        )
        Assert-Condition -Condition ($affected -eq 1) -Message 'Synthetic Slice 2 batch insert did not affect exactly one row.'
    }

    $instanceSnapshotSql = @'
INSERT INTO [dbo].[SqlInstanceDiscoverySnapshots]
([Id], [CustomerId], [ProjectId], [SqlInstanceId], [ImportBatchId], [ServerId], [InstanceName], [SqlVersion], [Edition], [Port], [ServiceStatus], [DiscoverySource], [LastDiscoveredAt], [ImportedAt])
VALUES
(@Id, @CustomerId, @ProjectId, @SqlInstanceId, @ImportBatchId, @ServerId, N'BASELINE_INSTANCE_A1', N'SQL Server 2022', N'Standard', @Port, N'Running', N'Synthetic assurance', SYSDATETIMEOFFSET(), SYSDATETIMEOFFSET());
'@
    $instanceSnapshotParameters = @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $instanceSnapshot),
        ...
        (New-SqlParameterSpec -Name '@SqlInstanceId' -Type UniqueIdentifier -Value $Ids.InstanceA1),
        (New-SqlParameterSpec -Name '@ImportBatchId' -Type UniqueIdentifier -Value $batchA),
        (New-SqlParameterSpec -Name '@ServerId' -Type UniqueIdentifier -Value $Ids.ServerA1),
        (New-SqlParameterSpec -Name '@Port' -Type Int -Value 1433)
    )
    Assert-Condition -Condition ((Invoke-SqlNonQuery -Sql $instanceSnapshotSql -Parameters $instanceSnapshotParameters) -eq 1) -Message 'Valid SQL Instance snapshot insert failed.'

    $databaseSnapshotSql = @'
INSERT INTO [dbo].[SqlDatabaseDiscoverySnapshots]
([Id], [CustomerId], [ProjectId], [SqlDatabaseId], [ImportBatchId], [SqlInstanceId], [Name], [SizeMb], [CompatibilityLevel], [RecoveryModel], [Collation], [Status], [ImportedAt])
VALUES
(@Id, @CustomerId, @ProjectId, @SqlDatabaseId, @ImportBatchId, @SqlInstanceId, N'BaselineDatabaseA1', @SizeMb, @CompatibilityLevel, N'Full', N'Latin1_General_100_CI_AS', N'Online', SYSDATETIMEOFFSET());
'@
    $databaseSnapshotParameters = @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $databaseSnapshot),
        (New-SqlParameterSpec -Name '@CustomerId' -Type UniqueIdentifier -Value $Ids.CustomerA),
        (New-SqlParameterSpec -Name '@ProjectId' -Type UniqueIdentifier -Value $Ids.ProjectA1),
        (New-SqlParameterSpec -Name '@SqlDatabaseId' -Type UniqueIdentifier -Value $Ids.DatabaseA1),
        (New-SqlParameterSpec -Name '@ImportBatchId' -Type UniqueIdentifier -Value $batchA),
        (New-SqlParameterSpec -Name '@SqlInstanceId' -Type UniqueIdentifier -Value $Ids.InstanceA1),
        (New-SqlParameterSpec -Name '@SizeMb' -Type BigInt -Value 1024),
        (New-SqlParameterSpec -Name '@CompatibilityLevel' -Type Int -Value 160)
    )
    Assert-Condition -Condition ((Invoke-SqlNonQuery -Sql $databaseSnapshotSql -Parameters $databaseSnapshotParameters) -eq 1) -Message 'Valid SQL Database snapshot insert failed.'

    $stagingSql = @'
INSERT INTO [dbo].[DiscoveryImportRows]
([Id], [CustomerId], [ProjectId], [ImportBatchId], [RowNumber], [SourceRecordId], [SourceType], [RawDataJson], [NormalizedHostname], [NormalizedInstanceName], [NormalizedDatabaseName], [Classification], [ProposedAction], [ValidationStatus], [ValidationMessagesJson], [MatchedEntityId], [MatchedSqlInstanceId], [MatchedSqlDatabaseId], [ReconciliationFingerprint], [ProposedChangesJson], [CreatedAt])
VALUES
(@Id, @CustomerId, @ProjectId, @ImportBatchId, 1, N'SYNTH-ROW-1', N'SqlInstanceCsv/v1', N'{"safe":"synthetic"}', N'DC-HOU-SQL01', N'BASELINE_INSTANCE_A1', NULL, N'Unchanged', N'Unchanged', N'Valid', N'[]', @ServerId, @SqlInstanceId, NULL, @Fingerprint, N'{}', SYSDATETIMEOFFSET());
'@
    $stagingParameters = @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $stagingRow),
        (New-SqlParameterSpec -Name '@CustomerId' -Type UniqueIdentifier -Value $Ids.CustomerA),
        (New-SqlParameterSpec -Name '@ProjectId' -Type UniqueIdentifier -Value $Ids.ProjectA1),
        (New-SqlParameterSpec -Name '@ImportBatchId' -Type UniqueIdentifier -Value $batchA),
        (New-SqlParameterSpec -Name '@ServerId' -Type UniqueIdentifier -Value $Ids.ServerA1),
        (New-SqlParameterSpec -Name '@SqlInstanceId' -Type UniqueIdentifier -Value $Ids.InstanceA1),
        (New-SqlParameterSpec -Name '@Fingerprint' -Type NChar -Value ('f' * 64) -Size 64)
    )
    Assert-Condition -Condition ((Invoke-SqlNonQuery -Sql $stagingSql -Parameters $stagingParameters) -eq 1) -Message 'Valid Slice 2 staging row insert failed.'

    $constraintFailures = New-Object System.Collections.ArrayList
    $badPort = @($instanceSnapshotParameters | ForEach-Object { $_ })
    $badPort[0] = New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value ([guid] '94000000-0000-0000-0000-000000000013')
    $badPort[6] = New-SqlParameterSpec -Name '@Port' -Type Int -Value 0
    $null = $constraintFailures.Add((Assert-SqlFailure -Name 'Snapshot port check' -Action { Invoke-SqlNonQuery -Sql $instanceSnapshotSql -Parameters $badPort } -ExpectedNumbers @(547) -ExpectedMessageFragment 'CK_SqlInstanceDiscoverySnapshots_Port'))

    $badSize = @($databaseSnapshotParameters | ForEach-Object { $_ })
    $badSize[0] = New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value ([guid] '94000000-0000-0000-0000-000000000014')
    $badSize[6] = New-SqlParameterSpec -Name '@SizeMb' -Type BigInt -Value -1
    $null = $constraintFailures.Add((Assert-SqlFailure -Name 'Snapshot size check' -Action { Invoke-SqlNonQuery -Sql $databaseSnapshotSql -Parameters $badSize } -ExpectedNumbers @(547) -ExpectedMessageFragment 'CK_SqlDatabaseDiscoverySnapshots_SizeMb'))

    $badCompatibility = @($databaseSnapshotParameters | ForEach-Object { $_ })
    $badCompatibility[0] = New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value ([guid] '94000000-0000-0000-0000-000000000015')
    $badCompatibility[7] = New-SqlParameterSpec -Name '@CompatibilityLevel' -Type Int -Value 79
    $null = $constraintFailures.Add((Assert-SqlFailure -Name 'Snapshot compatibility check' -Action { Invoke-SqlNonQuery -Sql $databaseSnapshotSql -Parameters $badCompatibility } -ExpectedNumbers @(547) -ExpectedMessageFragment 'CK_SqlDatabaseDiscoverySnapshots_CompatibilityLevel'))

    $crossScopeSnapshot = @($instanceSnapshotParameters | ForEach-Object { $_ })
    $crossScopeSnapshot[0] = New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value ([guid] '94000000-0000-0000-0000-000000000016')
    $crossScopeSnapshot[1] = New-SqlParameterSpec -Name '@CustomerId' -Type UniqueIdentifier -Value $Ids.CustomerB
    $crossScopeSnapshot[2] = New-SqlParameterSpec -Name '@ProjectId' -Type UniqueIdentifier -Value $Ids.ProjectB1
    $crossScopeSnapshot[4] = New-SqlParameterSpec -Name '@ImportBatchId' -Type UniqueIdentifier -Value $batchB
    $null = $constraintFailures.Add((Assert-SqlFailure -Name 'Cross-tenant snapshot canonical FK' -Action { Invoke-SqlNonQuery -Sql $instanceSnapshotSql -Parameters $crossScopeSnapshot } -ExpectedNumbers @(547) -ExpectedMessageFragment 'FK_SqlInstanceDiscoverySnapshots_SqlInstances_CustomerId_ProjectId_SqlInstanceId'))

    [System.Byte[]] $originalVersion = Invoke-SqlScalar -Sql 'SELECT [RowVersion] FROM [dbo].[ImportBatches] WHERE [Id] = @Id;' -Parameters @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $batchA)
    )
    $firstClaim = Invoke-SqlNonQuery -Sql 'UPDATE [dbo].[ImportBatches] SET [Notes] = N''first claim'' WHERE [Id] = @Id AND [RowVersion] = @RowVersion;' -Parameters @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $batchA),
        (New-SqlParameterSpec -Name '@RowVersion' -Type Binary -Value $originalVersion -Size 8)
    )
    $staleClaim = Invoke-SqlNonQuery -Sql 'UPDATE [dbo].[ImportBatches] SET [Notes] = N''stale claim'' WHERE [Id] = @Id AND [RowVersion] = @RowVersion;' -Parameters @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $batchA),
        (New-SqlParameterSpec -Name '@RowVersion' -Type Binary -Value $originalVersion -Size 8)
    )
    Assert-Condition -Condition ($firstClaim -eq 1 -and $staleClaim -eq 0) -Message 'ImportBatch rowversion did not allow exactly one optimistic-concurrency claimant.'

    $rolledBackSnapshot = [guid] '94000000-0000-0000-0000-000000000017'
    $transactionCount = [int] (Invoke-SqlScalar -Sql @'
SET XACT_ABORT ON;
BEGIN TRANSACTION;
INSERT INTO [dbo].[SqlInstanceDiscoverySnapshots]
([Id], [CustomerId], [ProjectId], [SqlInstanceId], [ImportBatchId], [ServerId], [InstanceName], [SqlVersion], [Edition], [Port], [ServiceStatus], [DiscoverySource], [LastDiscoveredAt], [ImportedAt])
VALUES
(@Id, @CustomerId, @ProjectId, @SqlInstanceId, @ImportBatchId, @ServerId, N'ROLLBACK_ONLY', N'SQL Server 2022', N'Standard', 1433, N'Running', N'Synthetic assurance', NULL, SYSDATETIMEOFFSET());
ROLLBACK TRANSACTION;
SELECT COUNT_BIG(*) FROM [dbo].[SqlInstanceDiscoverySnapshots] WHERE [Id] = @Id;
'@ -Parameters @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value $rolledBackSnapshot),
        (New-SqlParameterSpec -Name '@CustomerId' -Type UniqueIdentifier -Value $Ids.CustomerA),
        (New-SqlParameterSpec -Name '@ProjectId' -Type UniqueIdentifier -Value $Ids.ProjectA1),
        (New-SqlParameterSpec -Name '@SqlInstanceId' -Type UniqueIdentifier -Value $Ids.InstanceA1),
        (New-SqlParameterSpec -Name '@ImportBatchId' -Type UniqueIdentifier -Value $batchA),
    ...
    Assert-Condition -Condition ($transactionCount -eq 0) -Message 'Explicit SQL transaction rollback left a snapshot row behind.'

    $crossScopeStageAllowed = [int] (Invoke-SqlScalar -Sql @'
SET XACT_ABORT OFF;
BEGIN TRY
    BEGIN TRANSACTION;
    INSERT INTO [dbo].[DiscoveryImportRows]
    ([Id], [CustomerId], [ProjectId], [ImportBatchId], [RowNumber], [SourceRecordId], [SourceType], [RawDataJson], [Classification], [ValidationStatus], [CreatedAt])
    VALUES
    (@Id, @CustomerId, @ProjectId, @ImportBatchId, 999, N'CROSS-SCOPE', N'SqlInstanceCsv/v1', N'{"safe":"synthetic"}', N'Reject', N'Invalid', SYSDATETIMEOFFSET());
    ROLLBACK TRANSACTION;
    SELECT 1;
END TRY
BEGIN CATCH
...
END CATCH;
'@ -Parameters @(
        (New-SqlParameterSpec -Name '@Id' -Type UniqueIdentifier -Value ([guid] '94000000-0000-0000-0000-000000000022')),
        (New-SqlParameterSpec -Name '@CustomerId' -Type UniqueIdentifier -Value $Ids.CustomerB),
        (New-SqlParameterSpec -Name '@ProjectId' -Type UniqueIdentifier -Value $Ids.ProjectB1),
        (New-SqlParameterSpec -Name '@ImportBatchId' -Type UniqueIdentifier -Value $batchA)
    ))
    if ($crossScopeStageAllowed -eq 1) {
        $null = $Defects.Add([ordered]@{
            id = 'PH3SQL-S2-TST-001'
            severity = 'High'
            area = 'Tenant/project database integrity'
            finding = 'DiscoveryImportRows accepts CustomerId/ProjectId values that do not own ImportBatchId because the batch relationship is not a tenant-leading composite foreign key.'
            expected = 'The cross-scope staging insert must fail at the database boundary.'
            actual = 'The insert succeeded inside a transaction and was rolled back by the harness.'
        })
        Write-Evidence -Status FAIL -Message 'PH3SQL-S2-TST-001 reproduced: a cross-scope staging row can reference another tenant/project batch at the database boundary.'
    }
    else {
        Write-Evidence -Status PASS -Message 'Cross-scope staging-to-batch insert was rejected by SQL Server.'
    }

    $persistedCounts = [ordered]@{
        instanceSnapshots = [int] (Invoke-SqlScalar -Sql 'SELECT COUNT_BIG(*) FROM [dbo].[SqlInstanceDiscoverySnapshots] WHERE [ImportBatchId] = @BatchId;' -Parameters @((New-SqlParameterSpec -Name '@BatchId' -Type UniqueIdentifier -Value $batchA)))
        databaseSnapshots = [int] (Invoke-SqlScalar -Sql 'SELECT COUNT_BIG(*) FROM [dbo].[SqlDatabaseDiscoverySnapshots] WHERE [ImportBatchId] = @BatchId;' -Parameters @((New-SqlParameterSpec -Name '@BatchId' -Type UniqueIdentifier -Value $batchA)))
        stagingRows = [int] (Invoke-SqlScalar -Sql 'SELECT COUNT_BIG(*) FROM [dbo].[DiscoveryImportRows] WHERE [ImportBatchId] = @BatchId;' -Parameters @((New-SqlParameterSpec -Name '@BatchId' -Type UniqueIdentifier -Value $batchA)))
    }
    Assert-Condition -Condition ($persistedCounts.instanceSnapshots -eq 1 -and $persistedCounts.databaseSnapshots -eq 1 -and $persistedCounts.stagingRows -eq 1) -Message 'Slice 2 runtime persisted-count invariant failed.'

    $commonOwnerParameters = @(
        (New-SqlParameterSpec -Name '@CustomerId' -Type UniqueIdentifier -Value $Ids.CustomerA),
        (New-SqlParameterSpec -Name '@ProjectId' -Type UniqueIdentifier -Value $Ids.ProjectA1)
    )
    $null = Capture-ActualExecutionPlan -Name 'slice2-instance-history' -Sql @'
SELECT [Id], [ImportBatchId], [ImportedAt], [InstanceName]
FROM [dbo].[SqlInstanceDiscoverySnapshots]
WHERE [CustomerId] = @CustomerId AND [ProjectId] = @ProjectId AND [SqlInstanceId] = @CanonicalId
ORDER BY [ImportedAt] DESC, [Id];
'@ -Parameters ($commonOwnerParameters + @((New-SqlParameterSpec -Name '@CanonicalId' -Type UniqueIdentifier -Value $Ids.InstanceA1)))
    $null = Capture-ActualExecutionPlan -Name 'slice2-staging-instance-match' -Sql @'
SELECT [Id], [Classification], [ProposedAction]
FROM [dbo].[DiscoveryImportRows]
WHERE [CustomerId] = @CustomerId AND [ProjectId] = @ProjectId
  AND [NormalizedHostname] = N'DC-HOU-SQL01' AND [NormalizedInstanceName] = N'BASELINE_INSTANCE_A1';
'@ -Parameters $commonOwnerParameters
    $null = Capture-ActualExecutionPlan -Name 'slice2-idempotency-lookup' -Sql @'
SELECT [Id], [Status], [CommitResultJson]
FROM [dbo].[ImportBatches]
WHERE [CustomerId] = @CustomerId AND [ProjectId] = @ProjectId
  AND [CommitIdempotencyKeyHash] = @KeyHash;
'@ -Parameters ($commonOwnerParameters + @((New-SqlParameterSpec -Name '@KeyHash' -Type NChar -Value $idempotencyHash -Size 64)))

    Write-Evidence -Status PASS -Message 'Slice 2 SQL runtime checks completed: valid staging/history, checks, composite canonical isolation, rowversion concurrency, rollback and plans.'
    return [ordered]@{
        batchIds = @($batchA, $batchB)
        persistedCounts = $persistedCounts
        rejectedConstraintWrites = @($constraintFailures)
        firstRowVersionClaim = $firstClaim
        staleRowVersionClaim = $staleClaim
        transactionRollbackResidualRows = $transactionCount
        crossScopeStagingBatchInsertAllowed = ($crossScopeStageAllowed -eq 1)
    }
}
...
function Assert-RollbackBoundary {
    $history = @(Assert-MigrationHistory -Phase 'Rollback boundary' -Expected $ExpectedMigrations[0..3])
    $slice2TableCount = [int] (Invoke-SqlScalar -Sql @'
SELECT
    CASE WHEN OBJECT_ID(N'dbo.SqlInstanceDiscoverySnapshots', N'U') IS NULL
           AND OBJECT_ID(N'dbo.SqlDatabaseDiscoverySnapshots', N'U') IS NULL
         THEN 0 ELSE 1 END;
'@)
    Assert-Condition -Condition ($slice2TableCount -eq 0) -Message 'Slice 2 snapshot tables remained after rollback to the approved previous migration.'
    $slice2BatchColumnCount = [int] (Invoke-SqlScalar -Sql @'
SELECT COUNT_BIG(*)
FROM sys.columns
WHERE object_id = OBJECT_ID(N'dbo.ImportBatches', N'U')
  AND name IN (N'CommitIdempotencyKeyHash', N'CommitResultJson', N'RowVersion');
'@)
    Assert-Condition -Condition ($slice2BatchColumnCount -eq 0) -Message 'Slice 2 ImportBatches columns remained after rollback.'
    $slice2RowColumnCount = [int] (Invoke-SqlScalar -Sql @'
SELECT COUNT_BIG(*)
FROM sys.columns
WHERE object_id = OBJECT_ID(N'dbo.DiscoveryImportRows', N'U')
  AND name IN
  (
      N'MatchedSqlDatabaseId', N'MatchedSqlInstanceId', N'NormalizedDatabaseName',
      N'NormalizedInstanceName', N'ProposedAction', N'ReconciliationFingerprint'
  );
'@)
    Assert-Condition -Condition ($slice2RowColumnCount -eq 0) -Message 'Slice 2 DiscoveryImportRows columns remained after rollback.'
    $slice1TableCount = [int] (Invoke-SqlScalar -Sql @'
SELECT CASE WHEN OBJECT_ID(N'dbo.SqlInstances', N'U') IS NOT NULL
                 AND OBJECT_ID(N'dbo.SqlDatabases', N'U') IS NOT NULL
            THEN 2 ELSE 0 END;
'@)
    Assert-Condition -Condition ($slice1TableCount -eq 2) -Message 'Rollback removed an accepted Slice 1 SQL Inventory table.'
    Write-Evidence -Status PASS -Message 'Rollback reached AddInternalPrincipalAuditType, removed only Slice 2 schema, and retained accepted Slice 1 inventory.'
    return [ordered]@{
        history = $history
        slice2TablesPresent = $false
        slice2BatchColumnsPresent = $false
        slice2RowColumnsPresent = $false
        slice1InventoryTablesRetained = $true
    }
}
...
try {
    ...
    $fullTestOutput = Invoke-NativeCommand -Name 'Complete 209-test suite' -FilePath $dotnet.Source -Arguments @('test', $SolutionPath, '--configuration', 'Release', '--no-build', '--no-restore')
    $fullTestSummary = Assert-TestSummary -Name 'Complete suite' -Output $fullTestOutput -ExpectedPassed 209
    $Result.testSummaries['complete'] = $fullTestSummary

    $sqlDiscoveryUnitFilter = 'FullyQualifiedName~SqlDiscoveryCsvContractTests|FullyQualifiedName~IdentityAuthorizationTests'
    $sqlDiscoveryUnitOutput = Invoke-NativeCommand -Name 'Focused CSV and ADR-008 unit tests' -FilePath $dotnet.Source -Arguments @('test', (Join-Path $RepositoryRoot 'tests\api.unit\LgrTransformationMigration.Api.UnitTests.csproj'), '--configuration', 'Release', '--no-build', '--no-restore', '--filter', $sqlDiscoveryUnitFilter)
    $sqlDiscoveryUnitSummary = Assert-TestSummary -Name 'Focused CSV and ADR-008 unit suite' -Output $sqlDiscoveryUnitOutput -ExpectedPassed 60
    $Result.testSummaries['sqlDiscoveryUnit'] = $sqlDiscoveryUnitSummary

    $sqlDiscoveryIntegrationFilter = 'FullyQualifiedName~SqlDiscoveryImportApiTests'
    $sqlDiscoveryIntegrationOutput = Invoke-NativeCommand -Name 'Focused Slice 2 import integration tests' -FilePath $dotnet.Source -Arguments @('test', (Join-Path $RepositoryRoot 'tests\api.integration\LgrTransformationMigration.Api.IntegrationTests.csproj'), '--configuration', 'Release', '--no-build', '--no-restore', '--filter', $sqlDiscoveryIntegrationFilter)
    $sqlDiscoveryIntegrationSummary = Assert-TestSummary -Name 'Focused Slice 2 import integration suite' -Output $sqlDiscoveryIntegrationOutput -ExpectedPassed 11
    $Result.testSummaries['sqlDiscoveryIntegration'] = $sqlDiscoveryIntegrationSummary
...
'@
    ...
    $initialSlice2Schema = Assert-Slice2Schema -Phase 'Initial full migration'
    ...
    $Result.slice2RuntimeEvidence = Invoke-Slice2RuntimeTests -Ids $ids
...
'@ -Parameters @(
    ...
    $recoveredSlice2Schema = Assert-Slice2Schema -Phase 'Post-reapply recovery'
    ...
    Add-Step -Name 'Rollback/reapply recovery' -Outcome PASS -Detail 'The isolated database returned to the exact six-migration schema and is retained fully migrated for owner inspection.' -Evidence ([ordered]@{ rollback = $rollbackEvidence; finalHistory = $recoveredHistory })

    if ($Defects.Count -gt 0) {
        $Result.outcome = 'FAIL'
        $Result.outcomeReason = ("Authorised Slice 2 assurance completed with {0} reproducible defect(s); the isolated database remains fully migrated." -f $Defects.Count)
        $ExitCode = 1
        Write-Evidence -Status FAIL -Message $Result.outcomeReason
    }
    else {
        $Result.outcome = 'PASS'
        $Result.outcomeReason = 'All authorised PH3-SQL-001 Slice 2 SQL Server and regression assurance checks passed.'
        $ExitCode = 0
        Write-Evidence -Status PASS -Message 'PH3-SQL-001 Slice 2 assurance completed; the isolated database remains fully migrated for owner inspection.'
    }
}
catch {
