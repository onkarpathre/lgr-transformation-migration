...
export default function SqlInstanceDetailPage() {
  ...
  const actions = item.data && (hasPermission("dependency.read") || hasPermission("sql.inventory.update")) ? <div className="page-actions">{hasPermission("dependency.read") && <Link className="button secondary" href={`/planning/dependencies/assets/SqlInstance/${id}`}>Dependencies</Link>}{hasPermission("sql.inventory.update") && <button className="button secondary" onClick={() => setEditing(true)}>Edit</button>}{hasPermission("sql.inventory.delete") && <button className="button danger" disabled={acting} onClick={() => void archive()}>Archive</button>}</div> : undefined;
